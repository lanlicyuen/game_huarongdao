class HuaRongDao {    constructor() {
    ...
    selectPiece(piece) {
        // 清除之前的选择
        document.querySelectorAll('.piece').forEach(p => p.classList.remove('moving'));
    ...
    setupMoveListeners() {
        ...
        const moveHandler = (e) => {
            ...
            const x = Math.floor((e.clientX - rect.left) / this.cellSize.width);
            const y = Math.floor((e.clientY - rect.top) / this.cellSize.height);
            
            if (this.canMoveTo(this.selectedPiece, x, y)) {
            ...
            document.querySelectorAll('.piece').forEach(p => p.classList.remove('moving'));
            this.clearPossibleMoves();
            gameBoard.removeEventListener('click', moveHandler);
        };
